...
- **`main.py` 안전망(`boot.py`)**: 저장한 `main.py`가 문법 오류나 즉시 예외로 부팅에 실패하면, `boot.py`가 자동으로 `main.py.bak`을 복원합니다. **다만 `main.py`가 예외 없이 아주 이른 시점(모듈 최상단 등)에서 무한루프에 빠지는 경우는 이 자동 복구 대상이 아닙니다** — 이런 드문 경우엔 USB로 재업로드해야 합니다. (서비스 루프에 진입한 뒤 멈추는 경우는 하드웨어 워치독이 재부팅시키지만, 그 시점 이후로만 감지됩니다.)
...
3. `wifi_config.json`에 저장된 Wi-Fi 정보로 접속을 시도하고, 실패하면 `Pico-Dust-Setup`이라는 이름의 오프라인 핫스팟(AP)을 엽니다.
